...
void print_sieves(int n){
  ...
  //sieves pseudocode can be seen @https://en.wikipedia.org/wiki/Sieve_of_Eratosthenes#Pseudocode
    ...
    int lastPrime, numberOf4Diff = 0; // the number of times there are a difference of 4 and the last prime; 
    for(int i = 2; i < n; i++){
        if(list[i] == false){
            print_number(i);
            if(i-lastPrime == 4){
...
int main(int argc, char *argv[]){
  //if(argc == )
    print_sieves(1000000); // we can do slightly larger than 100 000 numbers in 2 seconds. this is a however, very dependant on ur ram/cpu
